        ...
        if ($quoteData->getItemsCount()!= count($responseItems)) {
            ...
            foreach ($quoteItems as $items) {
                $itemFound = array_search($items->getSku(), array_column($responseItems, 'sku'));
                   if ($itemFound === false) {
                    $this->_helper->debug('Cart items '.$items->getName().' does not match.');
                    $isCartupdated = true;
                    break;
                }elseif($items->getQty() != $responseItems[$itemFound]['quantity']){
                    $this->_helper->debug('Quantity of cart item - '.$items->getName().' does not match.');
                    $isCartupdated = true;
                    break;
                }
                continue;
