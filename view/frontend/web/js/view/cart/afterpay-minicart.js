    ...
    function ( $, priceUtils,mageUrl,customerData) {
        return function (config){
            function canDispayExpress(){
                ...
                if (!isAfterpayRestricted() && parseFloat(minicart_subtotal) >= parseFloat(config.afterpayConfig.minLimit) && parseFloat(minicart_subtotal) <= parseFloat(config.afterpayConfig.maxLimit) && (config.afterpayConfig.paymentActive)) {
